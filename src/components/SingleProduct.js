...
const SingleProduct = ({ prod }) => { //getting a product object as a prop
    ...
    //getting the cart from state, the dispatch function, and the convertPrice function
    const { state: { cart, currency }, dispatch, convertPrice } = CartState();
                ...
                <Card>
                    ...
                    <Card.Body className={`${theme === 'light' ? 'lightCard' : 'darkCard'}`}>
                        ...
                        <Card.Subtitle style={{ paddingBottom: 10 }}>
                            <span style={{ fontSize: '1.2rem' }}>£ {convertPrice(prod.price, currency)}</span>
                            ...
                            cart.some(p => p.id === prod.id) ?
                                ...
                                (<Button
                                    onClick={() => {
                                        dispatch({
                                            type: 'ADD_TO_CART',
                                            payload: { ...prod, qty: 1 }, // set qty to 1 when adding to cart
                                        });
